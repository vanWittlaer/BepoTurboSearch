...
### Added
- **Search results page integration**: LIKE-matched products now appear in the full search results page, not just the suggest dropdown
- **Duplicate prevention**: Products shown via LIKE matching are removed from Shopware's default results (including variants when parent is matched)

## [0.2.1] - 2025-12-03

### Added
- **Main products only option**: Filter LIKE matching to exclude variant products (configurable)
- **Configurable result limit**: Set maximum number of products returned by LIKE matching
- **Search term highlighting**: Matched portion of product number is highlighted in search results

### Changed
- **Improved product display**: Product name and price now on first line, product number on second line

## [0.2.0] - 2025-12-03

### Added
- **Product number LIKE matching**: Products are now matched against search terms using SQL LIKE '%search-term%' on the product number field
- **Plugin configuration**: New settings in Admin to enable/disable LIKE matching (default: disabled) and set minimum search term length (default: 3)
- **Priority display**: LIKE-matched products are displayed in a highlighted section above regular search results

## [0.1.1] - 2025-11-02

### Fixed
- **Product version field**: Added Required flag to ReferenceVersionField to ensure product_version_id is automatically populated when saving product references from Admin
